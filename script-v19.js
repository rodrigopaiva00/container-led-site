...
document.documentElement.classList.toggle('test-mode', SITE_CONFIG.TEST_MODE);
window.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.simulator-test-note').forEach((note) => {
    note.hidden = !SITE_CONFIG.TEST_MODE;
  });
});
...
function calculateSimulation() {
  ...
  const width = Math.max(1, Math.round(Number(data.get('largura')) || 1)), height = Math.max(1, Math.round(Number(data.get('altura')) || 1));
  const type = clean(data.get('ambiente')), application = clean(data.get('aplicacao'));
  const profiles = { outdoor:{label:'Painel outdoor',pitch:6,weight:42,power:.75}, indoor:{label:'Painel indoor',pitch:3,weight:28,power:.55}, testeira:{label:'Testeira para posto',pitch:6,weight:30,power:.50} };
  const profile=profiles[type]||profiles.outdoor, area=width*height;
